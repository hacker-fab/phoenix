...
class Curve:
    ...
    def extend_cv_points(self, points: Iterable[Iterable[float]]) -> None:
        """
        Bulk append multiple control points (optional helper) and re-sort.

        Parameters
        ----------
        points : Iterable[Iterable[float]]
            An iterable of 2-length iterables representing [x, y] pairs.
        """
        for p in points:
            x, y = p
            self._cv_points.append([float(x), float(y)])
        self.build_curve()

    # --------------------------------------------------------------------- #
    # Interpolation
    # --------------------------------------------------------------------- #
    def get_value(self, offset: float) -> float:
        """
        Perform linear interpolation for a given X offset.

        Parameters
        ----------
        offset : float
            X coordinate at which to evaluate the curve. This value is expected
            to be within the domain [min_x, max_x] covered by control points,
            but clamping behavior is provided for values outside the range.
        ...
        Notes
        -----
        Unlike the previous docstring that mentioned normalized 0..1 offsets,
        this implementation operates in the same unit space as the control
        point X values (so typically 0..x_max). Callers that want normalized
        sampling should scale accordingly (offset * x_max).
        """
        ...
        # Linear segment search (O(n)); acceptable for small point counts.
        for i in range(len(sorted_points) - 1):
            x1, y1 = sorted_points[i]
            x2, y2 = sorted_points[i + 1]
            if x1 <= offset <= x2:
                if x2 == x1:
                    # Degenerate segment; return start value.
                    return y1
                t = (offset - x1) / (x2 - x1)
                return y1 + t * (y2 - y1)

        # Fallback (should not reach here given earlier clamps)
        return 0.0
    ...
    def copy(self) -> "Curve":
        """
        Create a deep (logical) copy of the curve.

        Returns
        -------
        Curve
            New curve instance with duplicated control points and axis scales.
        """
        return Curve(
            x_max=self.x_max,
            y_min=self.y_min,
            y_max=self.y_max,
            _cv_points=[p.copy() for p in self._cv_points],
        )
