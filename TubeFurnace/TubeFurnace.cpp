...
char lcdBuffer[NUM_LCD_ROWS][NUM_LCD_CHARS] = {"tube furnace"};

// Values to keep a running average of
RunningAverage tempAvg(N_TEMP_AVG);
RunningAverage rampAvg(N_TEMP_RAMP_AVG);
...
// init clock
void resetClock() { clockStart = millis(); }
...
// init Time
void initTime() { currentTime = millis(); }
...
void getTime() { currentTime = millis(); }
...
void logData() {
    ...
    if (nextLogTime < currentTime) {
        ...
        Serial.print(tempAvg.getAverage());
        Serial.print(",");
        Serial.print("ramp:");
        Serial.print(rampRateF);
        Serial.print(",");
        Serial.print("rampAvg:");
        Serial.print(rampAvg.getAverage());
        Serial.print(",");
